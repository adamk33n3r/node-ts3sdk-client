...
/**
 * Wrapper for ts3client_getDefaultPlaybackDevice().
 */
NAN_METHOD(Playback::GetDefaultDevice)
{
    unsigned int error;
    char**       deviceDefault;
    ...
    if((error = ts3client_getDefaultPlaybackDevice(mode, &deviceDefault)) != ERROR_ok)
    {
        return Error::throwException(error);
    }
...
/**
 * Wrapper for ts3client_getDefaultPlayBackMode().
 */
NAN_METHOD(Playback::GetDefaultMode)
{
    unsigned int error;
    char*        modeDefault;
